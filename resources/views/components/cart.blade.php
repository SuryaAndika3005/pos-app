...
{{-- FIX: Format angka titik otomatis pada input paid_amount --}}
{{-- FIX: Pisah antara ketik nama manual vs pilih dari dropdown --}}

{{-- Tab Antrean --}}
<div class="flex items-center gap-1.5 mb-3 overflow-x-auto hide-scrollbar">
    <template x-for="(cart, i) in carts" :key="cart.id">
        <div class="flex items-center shrink-0">
            <div x-show="editingCartIndex !== i"
                 @click="activeCartIndex = i"
                 @dblclick="startRename(i)"
                 :class="activeCartIndex === i ? 'bg-indigo-500 text-white shadow-md' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700'"
                 class="px-3 py-1.5 rounded-xl text-xs font-bold cursor-pointer transition-all flex items-center gap-1.5">
                <span x-text="cart.name"></span>
                <button x-show="carts.length > 1" @click.stop="closeCart(i)"
                        class="opacity-50 hover:opacity-100 transition-opacity">✕</button>
            </div>
            <div x-show="editingCartIndex === i" x-cloak>
                <input x-ref="renameInput" x-model="editingName"
...
{{-- FIX: Saat kasir mengetik ulang nama secara manual (bukan pilih dari dropdown),
     customer_id direset agar transaksi tidak terikat ke pelanggan lain --}}
<div class="mb-3 relative" x-data="{ open: false, suggestions: [] }">
    <div class="flex items-center gap-1.5 mb-1 px-1">
        ...
        <span x-show="activeCart.customerName && !activeCart.customerId" class="ml-auto text-[9px] font-bold text-amber-500 bg-amber-50 dark:bg-amber-900/30 px-1.5 py-0.5 rounded">Tamu</span>
    </div>
    <input type="text"
           x-model="activeCart.customerName"
           @input.debounce.300ms="
               activeCart.customerId = null;
               activeCart.customerDebt = 0;
               searchCustomer($event.target.value, suggestions, () => open = suggestions.length > 0)
           "
           @focus="if(suggestions.length) open = true"
           @blur.debounce.200ms="open = false"
           placeholder="Ketik nama / telp, atau pilih dari daftar..."
